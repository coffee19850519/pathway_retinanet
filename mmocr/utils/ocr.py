...
def parse_args():
    ...
    parser.add_argument(
        '--config-dir',
        type=str,
        default='D:\PyCharmProject\mmocr\mmlab_mmocr\configs',
    ...
    if not os.path.samefile(args.config_dir, os.path.join(str(Path.cwd()))) and (args.det_config != ''
...
class MMOCR:        ###  MMOCR(det=None, recog='CRNN_TPS')
    ...
    def readtext(self,
        ...
        if self.detect_model and self.recog_model:
            det_recog_result = self.det_recog_kie_inference(self.detect_model, self.recog_model, kie_model=self.kie_model)
    ...
    def det_recog_pp(self, result):
        ...
        for arr, output, export, det_recog_result in zip(
            ...
            if not args.details:
                ...
                simple_res['text'] = [
                    (x['text'], x['text_score']) for x in det_recog_result['result']
                ]
    ...
    def det_recog_kie_inference(self, det_model, recog_model, kie_model=None):
        ...
        for filename, arr, bboxes, out_file in zip(self.args.filenames,
            ...
            for bbox in bboxes:
                ...
                # cv2.imwrite(r'F:/桌面文件/pathway_retinanet-pathway_retinanet_pairing/result/'+filename+str(bboxes.index(bbox))+ '.jpg', box_img)
...
if __name__ == '__main__':
    main()
    # print(os.path.join(str(Path.cwd()), 'configs/'))