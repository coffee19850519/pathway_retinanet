...
from new_mmocr import mmocr_without_det
...
def normalize_all_coordinates(prediction_instances, image_size):
    ...
    for row_idx in range(0, len(prediction_instances)):
        prediction_instances._set_value(prediction_instances.index[row_idx],
                                        ...
                                        normalize_rect_vertex(prediction_instances.iloc[row_idx]['coordinates'],
                                                              image_size))
...
def reorganize_outputs(el_output, body_output, image_ids, file_names, cfg):
    ...
    if el_model_instances.empty or body_instances.empty:
        element_instances = pd.DataFrame(columns=['image_id', 'file_name', 'category_id', 'bbox', 'score', 'ocr'])
        relation_head_instances = pd.DataFrame(columns=['image_id', 'file_name', 'category_id', 'bbox', 'score', 'ocr'])
        relation_body_instances = pd.DataFrame(columns=['image_id', 'file_name', 'category_id', 'bbox', 'score', 'ocr', 'head', 'tail'])
    ...
    relation_head_instances = el_model_instances.loc[(el_model_instances['score'] >= cfg.element_threshold) \
                                                     & (el_model_instances['category_id'] != cfg.element_list.index(
        'gene'))]
...
def save_hugo_slices_img(hugo_result, slices_path, current_img):
    import platform
    if platform.system() == 'Windows':
        current_img = current_img.split('\\')[-1].split('.jpg')[0]
    else:
        current_img = current_img.split('/')[-1].split('.jpg')[0]
    for index in range(len(hugo_result)):
        if hugo_result[index] == '-':
            remove_img = os.path.join(slices_path, current_img, current_img + '_' + str(index) + '.jpg')
            print('remove_img_path:', remove_img)
...
def get_ocr(current_image_file, article_gene_list, gene_name_list, data_folder, image_name, relation_body_instances,
            img_id, current_element_instances):
    '''
    ...
    # print(current_element_instances..columns.to_list())
    box_res['box'] = current_element_instances['bbox']
    instance_score = [i for i in current_element_instances['score']]

    if len(box_res['box']) == 0:
        relation_body_instances = pd.DataFrame(columns=['image_id', 'file_name',  'category_id', 'normalized_bbox',
                                                        'receptor', 'startor_bbox', 'receptor_bbox', 'relation_category'])
        return relation_body_instances
    print('img_name: ', current_image_file.split('/')[-1])
    # # Save text recognition results and picture slices
    # save_slice = True
    # ocr_results, coordinates_list = mmocr_without_det(recog='SATRN', img=current_image_file, element_bbox=box_res['box'],
    #                                                   output='my_visualize_result/' +
    #                                                          current_image_file.split('/')[-1], save_slices=save_slice)

    # Do not save text recognition results and picture slices
    save_slice = False
    ocr_results, coordinates_list = mmocr_without_det(recog='SATRN', img=current_image_file, element_bbox=box_res['box'], save_slices=save_slice)
    ocr_results = ocr_results[0]
    postprocessing_ocr_results = []
    seq_match_ratio_list = []
    max_match_str_list = []
    f = open('exHUGO_latest.json')
    hugo_dict = json.load(f)
    for idx, ocr_sample in enumerate(ocr_results):
        if '/' in ocr_sample:
            corrected_sample, seq_match_ratio, max_match_str = split_char(ocr_sample, '/', hugo_dict)
        else:
            corrected_sample, seq_match_ratio, max_match_str = corrected_processing_by_dict(hugo_dict, ocr_sample)
        postprocessing_ocr_results.append(corrected_sample)
        seq_match_ratio_list.append(seq_match_ratio)
        max_match_str_list.append(max_match_str)
        # postprocessing_ocr_results.append('akt')
    print('post ocr:', postprocessing_ocr_results)
    if save_slice:
        save_hugo_slices_img(postprocessing_ocr_results, 'slices_img_result', current_image_file)
    if len(set(postprocessing_ocr_results)) == 1 and postprocessing_ocr_results[0] == '-':
        relation_body_instances = pd.DataFrame(columns=['image_id', 'file_name',  'category_id', 'normalized_bbox',
                                                        'receptor', 'startor_bbox', 'receptor_bbox', 'relation_category'])
        return relation_body_instances
    ...
    for i in range(len(postprocessing_ocr_results)):
        ...
        json_dict['max_match_string'] = max_match_str_list[i]
        json_dict['post_gene_max_match_ratio'] = seq_match_ratio_list[i]
        json_dict['score'] = instance_score[i]
...
def get_receptor(relation_heads, current_relation_body_instances, gene_centers, processed_el_body_instances,
                 processed_genes):
    '''
    ...
    for i in range(0, len(relation_heads)):
        ...
        try:
            ...
            processed_el_body_instances['receptor_bbox'][current_relation_body_instances.index[i]] = \
                processed_el_body_instances['normalized_bbox'][processed_genes.index[min_j]]

        except:
            print('get_receptor except')
            print(min_j)
...
def get_startor(relation_tails, current_relation_body_instances, gene_centers, processed_el_body_instances,
                processed_genes, min_js):
    '''
    ...
    for i in range(0, len(relation_tails)):
        ...
        try:
            ...
            if tail_to_receptor < tail_to_closest_non_receptor:
                ...
                processed_el_body_instances['receptor'][current_relation_body_instances.index[i]] = ocr
                processed_el_body_instances['receptor_bbox'][current_relation_body_instances.index[i]] = \
                    processed_el_body_instances['normalized_bbox'][processed_genes.index[closes_entity]]

                ocr = processed_el_body_instances['ocr'][processed_genes.index[min_j]]
                processed_el_body_instances['startor'][current_relation_body_instances.index[i]] = ocr
                processed_el_body_instances['startor_bbox'][current_relation_body_instances.index[i]] = \
                    processed_el_body_instances['normalized_bbox'][processed_genes.index[min_j]]
            else:
                ocr = processed_el_body_instances['ocr'][processed_genes.index[closes_entity]]
                processed_el_body_instances['startor'][current_relation_body_instances.index[i]] = ocr
...
def save_visual(current_image_file, processed_el_body_instance, visuals_folder, image_name, ext):
    ...
    '''['image_id', 'file_name', 'category_id', 'normalized_bbox', 'bbox', 'score', 'ocr', 
    'head', 'tail', 'center', 'startor', 'receptor', 'startor_bbox', 'receptor_bbox', 'relation_category']'''
    ...
    for element_idx in range(0, len(processed_el_body_instance)):
        if (processed_el_body_instance.iloc[element_idx]['startor'] != '-') and (processed_el_body_instance.iloc[element_idx]['receptor'] != '-'):
            if processed_el_body_instance.iloc[element_idx]['category_id'] == 0:
                if processed_el_body_instance.iloc[element_idx]['score'] >= cfg.element_threshold:
                    if processed_el_body_instance.iloc[element_idx]['startor'] != processed_el_body_instance.iloc[element_idx]['receptor']:
                        cv2.polylines(img_copy,
                                      [normalize_rect_vertex(processed_el_body_instance.iloc[element_idx]['bbox'], image_size)],
                                      isClosed=True, color=(255, 0, 0), thickness=2)
            elif processed_el_body_instance.iloc[element_idx]['category_id'] == 1 and processed_el_body_instance.iloc[element_idx]['ocr'] != '-' \
                    and processed_el_body_instance.iloc[element_idx]['ocr'] != None:
                if processed_el_body_instance.iloc[element_idx]['score'] >= cfg.element_threshold:
                    cv2.polylines(img_copy,
                                  [normalize_rect_vertex(processed_el_body_instance.iloc[element_idx]['bbox'], image_size)],
        ...
        # if processed_el_body_instance.iloc[element_idx]['category_id'] == 1:
        #     # if processed_el_body_instance.iloc[element_idx]['score'] >= cfg.element_threshold:
        #     cv2.polylines(img_copy,
        #                   [normalize_rect_vertex(processed_el_body_instance.iloc[element_idx]['bbox'], image_size)],
        #                   isClosed=True, color=(0, 255, 0), thickness=2)
    for i in range(0, len(processed_el_body_instance[(processed_el_body_instance['category_id'] != 1) & (processed_el_body_instance['startor'] != '-')
                                          & (processed_el_body_instance['receptor'] != '-') &
                                                     (processed_el_body_instance['startor'] != None) & (processed_el_body_instance['receptor'] != None)])):
        head = processed_el_body_instance[(processed_el_body_instance['category_id'] != 1) & (processed_el_body_instance['startor'] != '-')
                                          & (processed_el_body_instance['receptor'] != '-') &
                                                     (processed_el_body_instance['startor'] != None) & (processed_el_body_instance['receptor'] != None)]['head']
        element_head = head.tolist()
        tail = processed_el_body_instance[(processed_el_body_instance['category_id'] != 1) & (processed_el_body_instance['startor'] != '-')
                                          & (processed_el_body_instance['receptor'] != '-') &
                                                     (processed_el_body_instance['startor'] != None) & (processed_el_body_instance['receptor'] != None)]['tail']
...
def get_startors_and_receptors(current_image_file, processed_el_body_instances):
    ...
    processed_el_body_instances, min_js = get_receptor(relation_heads, current_relation_body_instances, gene_centers,
                                                       processed_el_body_instances, processed_genes)
    processed_el_body_instances = get_startor(relation_tails, current_relation_body_instances, gene_centers,
                                              processed_el_body_instances, processed_genes, min_js)
...
def run_model(cfg, article_pd, **kwargs):
    ...
    DetectionCheckpointer(model=body_model, save_dir=configuration.OUTPUT_DIR).resume_or_load(cfg.relation_model,
                                                                                              resume=False)

    el_model = RegularTrainer.build_model(configuration)
    DetectionCheckpointer(model=el_model, save_dir=configuration.OUTPUT_DIR).resume_or_load(cfg.element_model,
                                                                                            resume=False)
    ...
    with inference_context(el_model.to(device)), inference_context(body_model.to(device)), torch.no_grad():
        for idx, inputs in enumerate(data_loader):
            ...
            print('doing image:', file_names)
            element_instances, relation_head_instances, relation_body_instances = reorganize_outputs(el_output,
                                                                                                     body_output,
                                                                                                     image_ids,
                                                                                                     file_names, cfg)
            if element_instances.empty or relation_head_instances.empty or relation_body_instances.empty:  ###
                p = file_names.split('img/')[0].replace('result', 'none_pathway')
                if not os.path.exists(p):
                    os.makedirs(p)
                shutil.copy(file_names, file_names.replace('result', 'none_pathway').replace('img/', ''))
            ...
            for current_image_file in file_list:
                ...
                current_relation_head_instances = relation_head_instances[
                    (relation_head_instances['file_name'] == current_image_file)]
                current_relation_body_instances = relation_body_instances[
                    (relation_body_instances['file_name'] == current_image_file)]
                ...
                processed_el_body_instances = get_ocr(current_image_file, article_gene_list, gene_name_list,
                                                      data_folder, image_name, current_relation_body_instances, img_id,
                                                      current_element_instances)
                if processed_el_body_instances.empty:  ###
                    p1 = file_names.split('img/')[0].replace('result', 'none_pathway')
                ...
                processed_el_body_instances = get_relationship_head(processed_el_body_instances,
                                                                    current_relation_head_instances)
                ...
                processed_el_body_instances = get_startors_and_receptors(current_image_file,
                                                                         processed_el_body_instances)
                ...
                results = result[
                    ["image_id", "file_name", "category_id", "bbox", "normalized_bbox", "score", "startor", "startor_bbox",
                     "relation_category",
        ...
        img_dir = file_names.split('img/')[0] + 'img/'
        if not any((file.endswith('.jpg')) for file in os.listdir(img_dir)):
            shutil.rmtree(img_dir.split('img/')[0])
            if os.path.exists(img_dir.split('img/')[0]):
                os.removedirs(img_dir.split('img/')[0])
