...
def mmocr_without_det(det=None, recog='SEG', img=None, element_bbox=None, output=None, save_slices=False):
    '''

    :param det: None
    :param recog: Text recognition model
    :param img: Processed pictures
    :param element_bbox: Text Box
    :param output: The path to store the text recognition results on a picture as large as the original picture
    :param save_slices: Whether to keep each text slice picture
    :return:
    '''
    img_crops = []
    boxs = []
    ...
    # print(img_crops)
    ocr = MMOCR(det=det, recog=recog)
    mmocr_results = ocr.readtext(img_crops, image_name=img_name, save_slices=save_slices)
    box_text_list = []
    # box_text_dit = {}
    # det_recog_result = [{'result': box_text_list}]

    for i in range(len(mmocr_results[0])):
        box_text_dit = {}  # 要写在里面 不然append到lit中的时候就会都变成最后一个append的元素
