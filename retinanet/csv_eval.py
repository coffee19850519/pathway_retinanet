    ...
    with torch.no_grad():
        ...
        for index in range(len(dataset)):
            ...
            if torch.cuda.is_available():
                scores, labels = retinanet(data['img'].permute(2, 0, 1).cuda().float().unsqueeze(dim=0))
            else:
                scores, labels = retinanet(data['img'].permute(2, 0, 1).float().unsqueeze(dim=0))
            scores = scores[0]
            labels = labels[0]


            # select indices which have file_path score above the threshold
            ...
            if indices.shape[0] > 0:
                ...
                image_detections = np.concatenate(
                    [np.expand_dims(image_scores, axis=1), np.expand_dims(image_labels, axis=1)], axis=1)
...
def evaluate(
    ...
    """ Evaluate file_path given dataset using file_path given retinanet.
    # Arguments
    ...
    for label in range(generator.num_classes()):
        ...
        if save_path != None:
            ...
            # giving file_path title to my graph
